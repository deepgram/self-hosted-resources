...
| engine.agentOverrides | object | `` | Per-engine-type resource overrides. Only applied when agent.enabled: true. Allows setting different GPU counts per engine type without affecting other resources. Supported keys: agent-speech-to-text, agent-text-to-speech, agent-end-of-turn  The agent-text-to-speech engine defaults to gpu: 2 because Aura-2 TTS requires exactly 2 GPUs to serve traffic: a single larger GPU does not substitute for two, and one Engine will not schedule work across more than two. STT and EOT engines are unaffected and continue to use the global engine.resources values (gpu: 1 by default).  Flux TTS uses a single GPU: if you enable `fluxTts` on a voice agent deployment, override the TTS engine to gpu: 1 as shown below, or the pod will request a second GPU it cannot use.  If you are not using Aura-2 and want to keep the TTS engine at 1 GPU, override:   engine:     agentOverrides:       agent-text-to-speech:         resources:           requests:             gpu: 1           limits:             gpu: 1 |
| engine.agentOverrides.agent-text-to-speech.resources.limits.gpu | int | `2` | Number of GPUs to limit for the TTS engine. Aura-2 requires 2. |
| engine.agentOverrides.agent-text-to-speech.resources.requests.gpu | int | `2` | Number of GPUs to request for the TTS engine. Aura-2 requires 2. |
...
| engine.modelManager.volumes.aws.efs.storageClass | object | `{"create":true,"name":null}` | Configuration for the EFS StorageClass |
| engine.modelManager.volumes.aws.efs.storageClass.create | bool | `true` | Specifies whether a StorageClass should be created |
| engine.modelManager.volumes.aws.efs.storageClass.name | string | `nil` | The name of the StorageClass to use. If not set and create is true, a name is generated using `<namePrefix>-aws-efs-sc`. Required when create is false. |
...
| engine.resources.requests.gpu | int | `1` | Number of GPUs to request. Aura and Aura-2 need 2 GPUs per Engine, and `engine.agentOverrides` does not apply outside a voice agent deployment, so a standalone TTS release must set this (and the matching limit) to 2 explicitly. |
| engine.resources.useNvidiaDevicePlugin | bool | `false` | When enabled, lets the NVIDIA device plugin handle GPU allocation, as opposed to mounting all GPUs into every Engine pod. When multiple Engine pods share a node, this ensures isolated utilization of GPUs. Note: with this enabled, each pod sees only its allocated GPUs (re-indexed from 0), so any `aura2.*.cudaVisibleDevices` values are interpreted relative to that set. Leave them at defaults and size the GPU request to match (specifically, Aura-2 needs 2 GPUs); the chart fails to render if the request is smaller than the number of devices listed in `aura2.*.cudaVisibleDevices`. When disabled (the default), the chart mounts all of the node's GPUs into every Engine pod. A GPU request smaller than the device count still runs in that mode, but it under-declares GPU usage to the scheduler, which can then place a second Engine on the same node to bind the same physical devices. See: https://github.com/NVIDIA/k8s-device-plugin |
...
| fluxTts | object | `` | Flux TTS specific configuration options. This is the text-to-speech model served on the `/v2/speak` endpoint, and is unrelated to `engine.flux`, which configures Flux turn-based streaming STT. Enable the endpoint itself with `api.features.speakV2` and `api.features.speakV2Streaming`. When `enabled` is true, the chart requires `uuid`, a non-zero `maxBatchSize`, and at least one of the two `api.features` flags above, and refuses to combine `fluxTts` with `aura2`. Invalid combinations fail at install time. |
| fluxTts.cudaVisibleDevices | string | `""` | GPUs the Flux TTS Engine may use, as CUDA device indices (e.g. `"0"`). Flux TTS binds to a single GPU, so on multi-GPU nodes pin each Engine to its own device. Leave empty to expose all GPUs to the Engine. Not needed when `engine.resources.useNvidiaDevicePlugin` handles GPU allocation. |
