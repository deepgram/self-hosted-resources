...
| billing.journal.size | string | `"1Gi"` | Size of the journal volume. The journal is plain text and typically only needs 1Gi. |
| billing.journal.storageClass | string | `""` | Storage class to use for the journal PVC. If empty, uses cluster default. |
...
| billing.updateStrategy.rollingUpdate | object | `` | For StatefulSets, we use maxUnavailable: 0 to ensure at least one billing pod is always available during updates. |
| billing.updateStrategy.rollingUpdate.maxUnavailable | int | `0` | The maximum number of extra Billing pods that can be created during a rollingUpdate. |
...
| licenseProxy.upstream | object | `` | Configuration for license-proxy to use billing container as upstream in airgapped deployments. Only applies when both licenseProxy.enabled and billing.enabled are true. |
| licenseProxy.upstream.useBilling | bool | `false` | Set to true to configure license-proxy to use the billing container as its upstream server instead of license.deepgram.com. |
