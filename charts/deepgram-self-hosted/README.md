...
| engine.resources.useNvidiaDevicePlugin | bool | `false` | When enabled, lets the NVIDIA device plugin handle GPU allocation, as opposed to mounting all GPUs into every Engine pod. When multiple Engine pods share a node, this ensures isolated utilization of GPUs. See: https://github.com/NVIDIA/k8s-device-plugin |
